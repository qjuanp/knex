...
class QueryCompiler_Oracle extends QueryCompiler {
  ...
  limit() {
    ...
    const { offset } = this.single;
    const hasLimit = limit || limit === 0 || limit === '0';
    const hasOffset = offset || offset === 0 || offset === '0';
    // has limit and offset// produce query
    if(hasLimit && hasOffset)
      return `offset ${this._getValueOrParameterFromAttribute('offset',offset)} fetch next ${this._getValueOrParameterFromAttribute('limit',limit)} rows only`;
    // has only limit // produce query
    if(hasLimit && !hasLimit)
    return `fetch next ${this._getValueOrParameterFromAttribute('limit',limit)} rows only`;
  }
  ...
  offset() {
    ...
    const hasLimit = limit || limit === 0 || limit === '0';
    const hasOffset = offset || offset === 0 || offset === '0';
    // haslimit and offset -> limit will implement the query
    if (hasLimit && hasOffset)
      return '';
    // has only offset -> respond the query
    if(hasOffset && !hasLimit)
      return `offset ${this._getValueOrParameterFromAttribute('offset',offset)}`;
  }
}
